defmodule Mastery.Core.Quiz do
  @moduledoc false
  alias Mastery.Core.{Response, Template, Question}
  ...
  def answer_question(quiz, %Response{correct: true} = response) do
    ...
    new_quiz |> maybe_advance(mastered?(new_quiz))
  end
  ...
  defp reset_record(%{current_question: question} = quiz) do
    ...
    |> Map.put(
      :record,
      quiz.record |> Map.delete(question.template.name)
    )
  end
  ...
  defp reset_used(%{current_question: question} = quiz) do
    ...
    |> Map.put(
      :used,
      quiz.used |> List.delete(question.template)
    )
  end
  ...
  defp remove_template_from_category(quiz) do
    ...
    new_templates =
      if new_category_templates = [] do
        quiz.templates |> Map.delete(template.category)
      else
  ...
  defp pick_current_question(quiz) do
    quiz |> Map.put(:current_question, select_a_random_question(quiz))
  end

  # pick random category, pick random question in category
